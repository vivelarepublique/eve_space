...
<script setup>
    ...
    const todayRef = ref(`今天是${today.getFullYear()}年${today.getMonth() + 1}月${today.getDate()}日 星期${weeks[today.getDay()]}`);
    ...
    const direction = ref(directions[random(iday, 2) % directions.length]);
    ...
    const ship = ref(pickRandom(ships, 3).join());
    ...
    const space = ref(pickRandom(spaces, 2).join());
</script>
