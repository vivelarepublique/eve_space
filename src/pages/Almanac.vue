...
export default {
  ...
  setup() {
    ...
    let todayString = `今天是${today.getFullYear()}年${today.getMonth() + 1}月${today.getDate()}日 星期${weeks[today.getDay()]}`;
    ...
    let goodList = [];
    let badList = [];
    let numGood = (random(iday, 98) % 3) + 2; //”宜“个数
    let numBad = (random(iday, 87) % 3) + 2; //”不宜“个数
    let eventArr = pickRandom(activities, numGood + numBad);
    ...
    function pickRandom(array, size) {
      ...
      for (let arr = 0; arr < array.length - size; arr++) {
        let index = random(iday, arr) % result.length;
